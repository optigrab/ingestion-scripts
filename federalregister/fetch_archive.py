...
fr_url = "https://www.federalregister.gov/api/v1/public-inspection-documents.json"
...
get_date = date(2016, 10, 7)
while get_date < date.today():
    print get_date.strftime("%Y-%m-%d")
    # Fetch current register documents
    r = requests.get(fr_url, params={'conditions[available_on]': get_date.strftime("%Y-%m-%d")})
    r.raise_for_status()
    data = r.json()
